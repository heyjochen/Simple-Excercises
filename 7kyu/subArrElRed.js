...
//P : arr with subarrays
//R: sum of set of numbers, start at arr[length-i], then arr[length-1], arr[length-2]
//E:
//P: get the length, arr[length-1-i], arr[length-1-i], arr[length-i]
...
const elementsSum = (arr, d = 0) =>
  arr.reduce(
    (a, b, i) =>
      a + (b[arr.length - 1 - i] !== undefined ? b[arr.length - 1 - i] : d),
    0
  );
