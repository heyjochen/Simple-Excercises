...
//P: a string, and a  width, optional character fill
//R: centered string. right and left are padded with fill, if fill % 2 is !0, then padding on the left is 1 character l;onger, if string is longer than width return string
//E: center('abc', 10, '_')  # returns "____abc___"
//P: first condition, fill is " " as default, if width % 2 !=0 then [optional.repeat(Math.floor(length/2)+1, string, optional.repeat(Math.floor(length/2)]

// default = ' ', if string.length > width return string, width % 2 === 0 then return fill.repeat(Math.floor(width/2)+1)
