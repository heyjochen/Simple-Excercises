...
//P: input n, arr from 1 to n
//R: last man standing, start at left and remove every other number till you reach end, then start from the right, until one number is left
//E: 1->9, last number is 6
//P: make array, while loop through it

function lastManStanding(n) {
  let arr = Array.from({ length: n }, (_, i) => i + 1);
  while (arr.length != 1) {
    arr = arr.filter((_, i) => i % 2 !== 0).reverse();
  }
  return arr[0];
}
