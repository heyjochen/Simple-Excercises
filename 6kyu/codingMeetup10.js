...
//P: array of objects
//R: new arr of objects with username to each object
//E: add a username property, firstName in lowerCase, first letter of lastName in lowerCase, birth year which is current year - age
//P: map through original array and return a new object that include userName

function addUsername(list) {
  const year = new Date().getFullYear();
  return list.map((dev) => ({
    ...dev,
    username: `${dev.firstName}${dev.lastName.slice(0, 1).toLowerCase()}${
      year - dev.age
    }`,
  }));
}
